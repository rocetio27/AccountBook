// This file was generated based on C:/Users/SpaceJockey27/Documents/Fuse/AccountBook/.uno/ux15/PageTab.g.uno.
// WARNING: Changes might be lost if you edit this file directly.

#pragma once
#include <Fuse.Animations.IResize.h>
#include <Fuse.Binding.h>
#include <Fuse.Controls.Panel.h>
#include <Fuse.Drawing.ISurfaceDrawable.h>
#include <Fuse.IActualPlacement.h>
#include <Fuse.INotifyUnrooted.h>
#include <Fuse.IProperties.h>
#include <Fuse.ISourceLocation.h>
#include <Fuse.ITemplateSource.h>
#include <Fuse.Node.h>
#include <Fuse.Scripting.IScriptObject.h>
#include <Fuse.Triggers.Actions.IHide.h>
#include <Fuse.Triggers.Actions.IShow.h>
#include <Fuse.Triggers.Actions-ea70af1f.h>
#include <Fuse.Visual.h>
#include <Uno.Collections.ICollection-1.h>
#include <Uno.Collections.IEnumerable-1.h>
#include <Uno.Collections.IList-1.h>
#include <Uno.Float4.h>
#include <Uno.UX.IPropertyListener.h>
namespace g{namespace Fuse{namespace Controls{struct Page;}}}
namespace g{namespace Fuse{namespace Controls{struct PageControl;}}}
namespace g{namespace Uno{namespace UX{struct Property1;}}}
namespace g{namespace Uno{namespace UX{struct Selector;}}}
namespace g{struct PageTab;}

namespace g{

// public partial sealed class PageTab :2
// {
::g::Fuse::Controls::Panel_type* PageTab_typeof();
void PageTab__ctor_7_fn(PageTab* __this, ::g::Fuse::Controls::PageControl* pageControl1, ::g::Fuse::Controls::Page* page1);
void PageTab__InitializeUX_fn(PageTab* __this);
void PageTab__get_Label_fn(PageTab* __this, uString** __retval);
void PageTab__set_Label_fn(PageTab* __this, uString* value);
void PageTab__New4_fn(::g::Fuse::Controls::PageControl* pageControl1, ::g::Fuse::Controls::Page* page1, PageTab** __retval);
void PageTab__SetLabel_fn(PageTab* __this, uString* value, uObject* origin);

struct PageTab : ::g::Fuse::Controls::Panel
{
    uStrong<uString*> _field_Label;
    uStrong< ::g::Fuse::Controls::PageControl*> pageControl;
    uStrong< ::g::Fuse::Controls::Page*> page;
    uStrong< ::g::Uno::UX::Property1*> temp_Value_inst;
    uStrong< ::g::Uno::UX::Property1*> this_Color_inst;
    static ::g::Uno::UX::Selector __selector0_;
    static ::g::Uno::UX::Selector& __selector0() { return PageTab_typeof()->Init(), __selector0_; }
    static ::g::Uno::UX::Selector __selector1_;
    static ::g::Uno::UX::Selector& __selector1() { return PageTab_typeof()->Init(), __selector1_; }

    void ctor_7(::g::Fuse::Controls::PageControl* pageControl1, ::g::Fuse::Controls::Page* page1);
    void InitializeUX();
    uString* Label();
    void Label(uString* value);
    void SetLabel(uString* value, uObject* origin);
    static PageTab* New4(::g::Fuse::Controls::PageControl* pageControl1, ::g::Fuse::Controls::Page* page1);
};
// }

} // ::g
